...
function update_12_13($db, $dbtype) {
	...
	if($dbtype=='mysql') {
		...
		$db->ex("CREATE TEMPORARY TABLE tags_backup (
 			id INTEGER,
 			name VARCHAR(50) NOT NULL,
...
#	}
	...
	if($dbtype=='mysql') {
		...
		$db->ex("CREATE TEMPORARY TABLE todolist_backup (
 			id INTEGER,
 			list_id INTEGER UNSIGNED NOT NULL default 0,
		...
		$db->ex("CREATE TABLE todolist ( id INTEGER PRIMARY KEY,
 			list_id INTEGER UNSIGNED NOT NULL default 0,
 			d_created INTEGER UNSIGNED NOT NULL default 0,
 			d_completed INTEGER UNSIGNED NOT NULL default 0,
 			compl TINYINT UNSIGNED NOT NULL default 0,
...
function update_131_14($db, $dbtype) {
	...
	if($dbtype=='mysql') {
		...
		$db->ex("CREATE TABLE {$db->prefix}tags (
			 ...
			 `name` VARCHAR(50) NOT NULL,
			 PRIMARY KEY(`id`),
			 UNIQUE KEY `name` (`name`)) CHARSET=utf8 ");
...
<tr><td>Table prefix:</td><td><input name=prefix value=\"mtt_\"></td></tr>
	...
	elseif ($dbtype == 'sqlite') {#sqlite
		try	{
			...
			$db->ex("CREATE TABLE {$db->prefix}tag2task ( tag_id INTEGER NOT NULL,
...
elseif($ver == $lastVer){
	exitMessage("Installed version does not require database update. <br><br> <b>Attention!</b> Delete Setup.php file for security reasons.");
}
else {
...
echo "Done<br><br> <b>Attention!</b> Delete setup.php for security reasons.";
printFooter();
