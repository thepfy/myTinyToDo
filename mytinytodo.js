...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		$(document.body).on('click','#mtt_filters .tag-filter .mtt-filter-close', function(){
		...
		$(document.body).on('click','#tagcloudcontent .tag', function(){
		...
		// Tabs
		$(document.body).on('click','#lists li.mtt-tab', function(event){
			if(event.metaKey || event.ctrlKey) {
				// hide the tab
				hideTab(this);
				return false;
		...
		$(document.body).on('click','#lists li.mtt-tab .list-action', function(){
			listMenu(this);
		...
		$(document.body).on('click','#alltags .tag', function(){
		...
		$("#edittags").autocomplete({source:'ajax.php?suggestTags', scroll: false, multiple: true, selectFirst:false, max:8, extraParams:{list:function(){ var taskId = document.getElementById('taskedit_form').id.value; return taskList[taskId].listId; }}});
		...
		$(document.body).on('dblclick','#tasklist li', function(){
			//clear selection
		...
		$(document.body).on('click','#tasklist .taskactionbtn', function(){
			var id = parseInt(getLiTaskId(this));
			if(id) taskContextMenu(this, id);
			return false;
		});

		$(document.body).on('click','#tasklist input[type=checkbox]', function(){
			var id = parseInt(getLiTaskId(this));
			if(id) completeTask(id, this);
			//return false;
		});

		$(document.body).on('click','#tasklist .task-toggle', function(){
			var id = getLiTaskId(this);
			if(id) $('#taskrow_'+id).toggleClass('task-expanded');
			return false;
		});

		$(document.body).on('click','#tasklist .tag', function(event){
		...
		if(!this.options.touchDevice) {
			$(document.body).on('mouseover mouseout','#tasklist .task-prio', function(event){
				var id = parseInt(getLiTaskId(this));
				if(!id) return;
		...
		$(document.body).on('click','#tasklist .mtt-action-note-cancel', function(){
			var id = parseInt(getLiTaskId(this));
			if(id) cancelTaskNote(id);
			return false;
		});

		$(document.body).on('click','#tasklist .mtt-action-note-save', function(){
			var id = parseInt(getLiTaskId(this));
		...
		if(this.options.tagPreview) {
			$(document.body).on('mouseover mouseout','#tasklist .tag', function(event){
		...
		$(document).ajaxError(function(event, request, settings){$('#msg')
			var errtxt;
		...
		$(document.body).on('submit',"#settings_form", function() {
			saveSettings(this);
			return false;
		});

		$(document.body).on('click',".mtt-back-button", function(){ _mtt.pageBack(); this.blur(); return false; } );

		// Commeting out because this triggers for some reason
